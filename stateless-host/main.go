...
func UnsafeReadFromStdin(n int) []byte {
	buf := make([]byte, n)
	_, err := io.ReadFull(os.Stdin, buf)
	if err != nil {
		panic(fmt.Sprintf("failed to read %d bytes from stdin: %v", n, err))
	}
	return buf
}
...
func Read[T any]() T {
        ...
        lengthBytes := UnsafeReadFromStdin(8)
        length := uint64(0)
        for i := 0; i < 8; i++ {
        ...
        data := UnsafeReadFromStdin(int(length))
